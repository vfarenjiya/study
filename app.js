...
function expandCurriculum() {
  ...
  // Build ONE lesson per individual topic, so all 133 entries from data.js
  // show up as distinct, trackable items (not merged into a handful of
  // combined per-subject lessons).
  TOPICS.forEach((topic, idx) => {
    const meta = SUBJECT_META[topic.s] || { name: topic.s.toUpperCase() };

    let contentHtml = `
      <div style="margin-bottom: 16px;">
        <span class="module-tag" style="background:${meta.color}22; color:${meta.color};">${meta.name}</span>
      </div>
    ...
    // Cross-reference with the deep MATH_DATABASE. If it exists, append it here!
