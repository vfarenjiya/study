...
const Fetcher = {
    ...
    async fetchPage(url){ for(const pf of this.proxies){ try{ const r=await fetch(pf(url),{signal:AbortSignal.timeout(15000), headers:{'Accept-Encoding':'gzip, deflate, br'}}); if(r.ok)return await r.text(); }catch(e){} } throw new Error('Could not reach YouTube. Check your connection and try again.'); },
    ...
    parsePlaylist(html, pid){ const m=html.match(/var\s+ytInitialData\s*=\s*(\{.*?\});/s); if(!m)throw new Error('Could not read playlist page. It may be private or unavailable.'); const d=JSON.parse(m[1]); let title='Unknown', author='Unknown', videos=[]; try{ const h=d?.header?.playlistHeaderRenderer; if(h){title=h.title?.simpleText||title; author=h.ownerText?.runs?.[0]?.text||author;} const tabs=d?.contents?.twoColumnBrowseResultsRenderer?.tabs; if(tabs)for(const tab of tabs){ const cs=tab?.tabRenderer?.content?.sectionListRenderer?.contents; if(!cs)continue; for(const s of cs){ const items=s?.itemSectionRenderer?.contents?.[0]?.playlistVideoListRenderer?.contents; if(!items)continue; for(const it of items){ const v=it?.playlistVideoRenderer; if(v)videos.push({id:v.videoId, title:v.title?.runs?.[0]?.text||'Untitled', thumbnail:`https://i.ytimg.com/vi/${v.videoId}/mqdefault.jpg`}); } } } }catch(e){} if(!videos.length)throw new Error('No videos found. The playlist may be empty, private, or deleted.'); return {id:pid, title, author, thumbnail:`https://i.ytimg.com/vi/${videos[0].id}/hqdefault.jpg`, videos, watched:[], tags:[]}; },
    parseChannel(html){ const m=html.match(/var\s+ytInitialData\s*=\s*(\{.*?\});/s); if(!m)throw new Error('Could not read channel page. It may not exist or is unavailable.'); const d=JSON.parse(m[1]); let pls=[], name='Unknown'; try{ const h=d?.header?.c4TabbedHeaderRenderer; if(h)name=h.title||name; const tabs=d?.contents?.twoColumnBrowseResultsRenderer?.tabs; if(tabs)for(const tab of tabs){ const cs=tab?.tabRenderer?.content?.sectionListRenderer?.contents; if(!cs)continue; for(const s of cs){ const items=s?.itemSectionRenderer?.contents?.[0]?.gridRenderer?.items; if(!items)continue; for(const it of items){ const pl=it?.gridPlaylistRenderer; if(pl)pls.push({id:pl.playlistId, title:pl.title?.runs?.[0]?.text||'Untitled', count:pl.videoCountShortText?.simpleText||'?'}); } } } }catch(e){} if(!pls.length) throw new Error('No public playlists found for this channel.'); return {channelName:name, playlists:pls}; }
};
...
const renderChannelResults = (r, tags) => {
    ...
    r.playlists.forEach(pl => {
        ...
        d.querySelector('button').addEventListener('click', async e => {
            ...
            } catch(err) { btn.textContent = 'Err'; btn.title = err.message || 'Failed to add'; }
        });
...
(async function initApp() {
    ...
    applyTheme(s.theme); applyAccent(s.accent || 'red');
