...
// Returns { subjectCode, question, index } for whichever mode we're in (quiz or review)
...
// Keyboard shortcuts: 1-4 select an option, Enter submits/advances
...
// Force-rebuild curriculum from data.js (all data should be loaded by now)
if (typeof buildCurriculum !== 'undefined') {
  try {
    const newCurr = buildCurriculum();
    if (Object.keys(newCurr).length > 0) {
      CURRICULUM = newCurr;
      SUBJECTS_ORDER = Object.keys(CURRICULUM);
    }
  } catch (e) {
    console.warn('Curriculum build failed:', e.message);
  }
}
