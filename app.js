...
function renderQuiz() {
  // Check for review-queue completion BEFORE computing a question ref:
  // once currentReviewIdx runs past the end of the queue, getActiveQuestionRef()
  // returns null for the exact same reason as "no quizzes for this module", so
  // this case must be handled first or it incorrectly falls into the generic
  // "no ref" branch below and shows the (wrong) success screen instead of the
  // dashboard.
  if (state.mode === 'review' && state.currentReviewIdx >= state.reviewQueue.length) {
    state.mode = 'dashboard';
    saveState();
    render();
    return;
  ...
  if (!ref) {
      // If there are no quizzes for this module, skip directly to success
      state.mode = 'success';
      saveState();
      render();
      return;
...
// Initialize on load
syncTheme();
loadState();        // Load progress states
