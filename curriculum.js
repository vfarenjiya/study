const CURRICULUM = {
  ...
  LA: {
    ...
    lessons: [
      ...
      {
        id: 'la-06',
        title: 'Special Matrices & LU Decomposition',
        content: `
          <p>GATE frequently tests whether you can recognize a matrix's "type" from its defining property and immediately recall the consequence.</p>

          <div class="def-box">
            <h4>Orthogonal Matrix</h4>
            <p>A square matrix Q is <strong>orthogonal</strong> if its columns (and rows) are orthonormal: $$Q^TQ = QQ^T = I \\quad\\Rightarrow\\quad Q^{-1} = Q^T$$</p>
            <p>Orthogonal matrices preserve length and angles — they represent pure rotations/reflections. $\\det(Q) = \\pm 1$.</p>
          </div>

          <div class="def-box">
            <h4>Idempotent Matrix</h4>
            <p>A matrix P is <strong>idempotent</strong> if $$P^2 = P$$</p>
            <p>Applying it twice does nothing new — the classic example is a <strong>projection matrix</strong>.</p>
          </div>

          <div class="theorem-box">
            <h5>Projection Matrix</h5>
            <p>The matrix that projects any vector $b$ onto the column space of A is: $$P = A(A^TA)^{-1}A^T$$</p>
            <p>Projection matrices are always symmetric ($P^T = P$) and idempotent ($P^2 = P$) — projecting an already-projected vector changes nothing.</p>
            <p><strong>Why it matters:</strong> this P is exactly what produces $\\hat{y} = Pb$, the fitted values in least-squares linear regression.</p>
          </div>

          <div class="example-box">
            <h4>Eigenvalues of Idempotent & Orthogonal Matrices</h4>
            <p>If $P^2 = P$, eigenvalues of P can only be 0 or 1 (from $\\lambda^2 = \\lambda$).</p>
            <p>If Q is orthogonal, every eigenvalue $\\lambda$ satisfies $|\\lambda| = 1$ (possibly complex).</p>
          </div>

          <div class="def-box">
            <h4>Partition (Block) Matrices</h4>
            <p>A matrix can be split into sub-blocks and multiplied block-by-block just like ordinary matrix multiplication, provided block dimensions are compatible: $$\\begin{pmatrix}A & B\\\\C & D\\end{pmatrix}\\begin{pmatrix}E\\\\F\\end{pmatrix} = \\begin{pmatrix}AE+BF\\\\CE+DF\\end{pmatrix}$$</p>
            <p>Useful for exploiting structure (e.g., block-diagonal matrices) to simplify determinant and inverse computations.</p>
          </div>

          <div class="theorem-box">
            <h5>LU Decomposition</h5>
            <p>Any (well-behaved) square matrix A can be factored as $$A = LU$$ where L is lower-triangular (with 1's on the diagonal) and U is upper-triangular — the direct byproduct of Gaussian elimination without row swaps.</p>
            <p><strong>Why it matters:</strong> once you have L and U, solving $Ax=b$ becomes two cheap triangular solves (forward then backward substitution) instead of one expensive elimination — critical when solving the same system for many different b's.</p>
            <p>If row swaps are needed, we get $PA = LU$ where P is a permutation matrix.</p>
          </div>

          <p style="margin-top: 4px; color: var(--muted);">⚠️ <strong>Common pitfall:</strong> Don't confuse idempotent ($P^2=P$) with orthogonal ($Q^TQ=I$) — a matrix can be both only if it equals the identity (or is a symmetric idempotent projection whose nonzero eigenspace is everything). Projection matrices are idempotent and symmetric, but generally <em>not</em> orthogonal.</p>
        `
      }
    ],
    quiz: [
      ...
      {
        q: 'If matrix P satisfies P² = P, what are its only possible eigenvalues?',
        opts: ['-1 and 1', '0 and 1', 'Any real number', '0 only'],
        ans: 1,
        exp: 'From $\\lambda^2 v = \\lambda v$ (applying P² = P to an eigenvector), we get $\\lambda^2 = \\lambda$, so $\\lambda \\in \\{0, 1\\}$. This is the defining property of idempotent (projection) matrices.'
      },
      {
        q: 'For an orthogonal matrix Q, which relation always holds?',
        opts: ['Q² = Q', 'Q⁻¹ = Qᵀ', 'Q = Qᵀ', 'det(Q) = 0'],
        ans: 1,
        exp: 'By definition, an orthogonal matrix satisfies $Q^TQ = I$, which directly gives $Q^{-1} = Q^T$.'
      },
      {
        q: 'What is the main practical benefit of LU decomposition when solving Ax = b for many different b vectors?',
        opts: ['It reduces the rank of A', 'It avoids computing eigenvalues', 'Each solve becomes two cheap triangular substitutions instead of full elimination', 'It guarantees A is invertible'],
        ans: 2,
        exp: 'Once A = LU is computed once, solving for any new b only requires a forward substitution (Ly = b) and a backward substitution (Ux = y), both O(n²) instead of repeating O(n³) elimination.'
      }
    ]
  },
  ...
  DBMS: {
    ...
    lessons: [
      ...
      {
        id: 'dbms-06',
        title: 'Data Transformation & Multidimensional Modeling',
        content: `
          <p>Before data reaches an analysis or ML pipeline, it's usually reshaped. These transformation techniques are as much a GATE DA topic as they are practical preprocessing steps.</p>

          <div class="def-box">
            <h4>Normalization (Data Scaling)</h4>
            <p>Not to be confused with relational normalization — here it means rescaling numeric attributes to a common range.</p>
            <p><strong>Min-Max normalization:</strong> $$x' = \\frac{x - min}{max - min}$$ maps values to $[0,1]$.</p>
            <p><strong>Z-score normalization:</strong> $$x' = \\frac{x - \\mu}{\\sigma}$$ centers data to mean 0, std-dev 1.</p>
          </div>

          <div class="def-box">
            <h4>Discretization</h4>
            <p>Converts a continuous attribute into discrete intervals/bins — e.g., "age" into "child / adult / senior".</p>
            <p><strong>Equal-width binning:</strong> divides the range into intervals of equal size (sensitive to outliers).</p>
            <p><strong>Equal-frequency (equal-depth) binning:</strong> each bin gets roughly the same number of data points.</p>
          </div>

          <div class="theorem-box">
            <h5>Sampling & Compression</h5>
            <p><strong>Sampling:</strong> selecting a representative subset of data for faster analysis — simple random sampling (with/without replacement), stratified sampling (preserves subgroup proportions).</p>
            <p><strong>Data compression:</strong> reduces storage/transmission size. <strong>Lossless</strong> (e.g., run-length encoding) reconstructs data exactly; <strong>lossy</strong> (e.g., discretization itself, or dimensionality reduction like PCA) discards some detail for greater compression.</p>
          </div>

          <div class="example-box">
            <h4>Multidimensional Data Model: Schema Types</h4>
            <p><strong>Star schema:</strong> one fact table connected directly to denormalized dimension tables — fast joins, some redundancy.</p>
            <p><strong>Snowflake schema:</strong> dimension tables are further normalized into sub-dimension tables — saves space, but requires more joins per query.</p>
            <p><strong>Fact constellation (galaxy schema):</strong> multiple fact tables share dimension tables — models several related business processes at once.</p>
          </div>

          <div class="def-box">
            <h4>Concept Hierarchies & Measures</h4>
            <p>A <strong>concept hierarchy</strong> defines a sequence of mappings from low-level to higher-level, more general concepts — e.g., Day → Month → Quarter → Year, or City → State → Country. Roll-up/drill-down in OLAP move along these hierarchies.</p>
            <p><strong>Measures</strong> in a fact table are classified by how they aggregate along dimensions:</p>
            <p>• <strong>Distributive:</strong> can be computed by combining sub-aggregates (e.g., SUM, COUNT, MIN, MAX).</p>
            <p>• <strong>Algebraic:</strong> computed from a bounded number of distributive aggregates (e.g., AVG = SUM/COUNT).</p>
            <p>• <strong>Holistic:</strong> no constant-size sub-aggregate suffices (e.g., MEDIAN, MODE) — must revisit the full dataset.</p>
          </div>

          <p style="margin-top: 4px; color: var(--muted);">⚠️ <strong>Common pitfall:</strong> Don't confuse "normalization" here (rescaling numeric ranges for data mining) with relational "normalization" (1NF/2NF/3NF/BCNF for eliminating redundancy) — GATE questions rely on context to distinguish the two.</p>
        `
      }
    ],
    quiz: [
      ...
      {
        q: 'Which data warehouse schema further normalizes dimension tables into sub-dimensions?',
        opts: ['Star schema', 'Snowflake schema', 'Fact constellation', 'Flat schema'],
        ans: 1,
        exp: 'A snowflake schema normalizes dimension tables into related sub-tables, trading extra joins for reduced redundancy — unlike the denormalized star schema.'
      },
      {
        q: 'MEDIAN is an example of which type of aggregate measure?',
        opts: ['Distributive', 'Algebraic', 'Holistic', 'Additive'],
        ans: 2,
        exp: 'Holistic measures like MEDIAN and MODE cannot be computed from a fixed-size set of sub-aggregates — you need the full underlying dataset, unlike SUM (distributive) or AVG (algebraic).'
      }
    ]
  },
  ...
  ML: {
    ...
    lessons: [
      ...
      {
        id: 'ml-06',
        title: 'kNN, Naive Bayes & Linear Discriminant Analysis',
        content: `
          <p>These three classifiers make very different assumptions about how data is generated, and GATE loves testing that distinction.</p>

          <div class="def-box">
            <h4>K-Nearest Neighbours (kNN)</h4>
            <p>A completely non-parametric, "lazy" learner: to classify a new point, find the K closest training points (by some distance metric, usually Euclidean) and take a majority vote (classification) or average (regression).</p>
            <p>No training phase at all — all the cost is at prediction time, $O(n)$ per query for a naive implementation.</p>
          </div>

          <div class="example-box">
            <h4>Choosing K</h4>
            <p>Small K (e.g. K=1) → low bias, high variance, sensitive to noise. Large K → high bias, low variance, smoother decision boundary. K is typically chosen via cross-validation; using an odd K avoids ties in binary classification.</p>
          </div>

          <div class="theorem-box">
            <h5>Naive Bayes Classifier</h5>
            <p>Applies Bayes' theorem with a strong ("naive") assumption: all features are conditionally independent given the class. $$P(y|x_1,...,x_n) \\propto P(y)\\prod_{i=1}^n P(x_i|y)$$</p>
            <p>Despite the unrealistic independence assumption, it performs surprisingly well in practice (e.g., spam filtering) and trains extremely fast — just count frequencies.</p>
          </div>

          <div class="def-box">
            <h4>Linear Discriminant Analysis (LDA)</h4>
            <p>A generative classifier that assumes each class's features follow a multivariate Gaussian with a <strong>shared covariance matrix</strong> across classes. This shared-covariance assumption is what makes the resulting decision boundary <strong>linear</strong> (unlike QDA, which allows per-class covariances and gives quadratic boundaries).</p>
            <p>LDA also doubles as a dimensionality-reduction technique: it projects data onto the direction(s) that best separate class means relative to within-class scatter.</p>
          </div>

          <p style="margin-top: 4px; color: var(--muted);">⚠️ <strong>Common pitfall:</strong> kNN is a discriminative, instance-based method with zero training cost but expensive prediction; Naive Bayes and LDA are generative — they model $P(x|y)$ and are cheap at both training and prediction time.</p>
        `
      },
      {
        id: 'ml-07',
        ...
        content: `
          <div class="def-box">
            ...
            <p>Ordinary least squares minimizes $\\|y - X\\theta\\|^2$. <strong>Ridge regression</strong> adds an L2 penalty: $$\\min_\\theta \\|y - X\\theta\\|^2 + \\lambda\\|\\theta\\|^2$$</p>
            <p>Closed-form solution: $$\\hat\\theta_{ridge} = (X^TX + \\lambda I)^{-1}X^Ty$$</p>
            <p>Adding $\\lambda I$ also fixes a practical problem: it makes $X^TX + \\lambda I$ invertible even when $X^TX$ is singular (e.g., with correlated/redundant features).</p>
          </div>

          <div class="example-box">
            <h4>Effect of λ</h4>
            <p>$\\lambda = 0$ recovers plain OLS. As $\\lambda \\to \\infty$, all coefficients shrink toward 0 (but never exactly 0, unlike Lasso) — increasing bias but reducing variance.</p>
          </div>

          <div class="theorem-box">
            <h5>Cross-Validation Methods</h5>
            <p><strong>k-Fold CV:</strong> split data into k equal folds; train on k−1, validate on the remaining fold; rotate through all k folds and average the validation scores.</p>
            <p><strong>Leave-One-Out CV (LOO-CV):</strong> the special case k = n — each fold is a single data point. Nearly unbiased estimate of test error, but expensive (n model fits) and high variance across folds since training sets overlap almost entirely.</p>
          </div>

          <div class="def-box">
            <h4>Bias-Variance Tradeoff in Model Selection</h4>
            <p>Smaller k (e.g., k=5 or 10) trains on less data per fold → slightly more biased but computationally cheap and lower variance than LOO-CV. This is why 5-fold or 10-fold CV is the practical default, with LOO reserved for very small datasets.</p>
          </div>

          <p style="margin-top: 4px; color: var(--muted);">💡 <strong>Exam tip:</strong> λ in ridge regression is itself a hyperparameter — it's chosen using cross-validation on a held-out set, never estimated from the same data used to fit θ.</p>
        `
      },
      {
        id: 'ml-08',
        title: 'Multi-Layer Perceptron & Feed-Forward Networks',
        content: `
          <div class="def-box">
            <h4>From Perceptron to MLP</h4>
            <p>A single perceptron computes $z = w^Tx + b$ and applies an activation function — it can only learn linearly separable functions (famously, it cannot represent XOR).</p>
            <p>A <strong>Multi-Layer Perceptron (MLP)</strong> stacks layers of such units with nonlinear activations between them, allowing it to approximate arbitrarily complex (non-linear) decision boundaries — this is the <strong>Universal Approximation Theorem</strong>.</p>
          </div>

          <div class="theorem-box">
            <h5>Feed-Forward Computation</h5>
            <p>Information flows strictly forward, layer by layer, with no cycles: $$a^{(l)} = f(W^{(l)}a^{(l-1)} + b^{(l)})$$</p>
            <p>Common activation functions f: <strong>Sigmoid</strong> $\\frac{1}{1+e^{-z}}$ (saturates, vanishing gradients), <strong>ReLU</strong> $\\max(0,z)$ (fast, avoids vanishing gradient for positive inputs), <strong>tanh</strong> (zero-centered version of sigmoid).</p>
          </div>

          <div class="example-box">
            <h4>Why Depth Matters</h4>
            <p>Without a nonlinear activation, stacking any number of linear layers collapses to a single linear transformation. Nonlinearities between layers are what let an MLP represent non-linearly-separable functions like XOR with just one hidden layer.</p>
          </div>

          <div class="def-box">
            <h4>Backpropagation (Conceptual)</h4>
            <p>Training uses gradient descent on a loss function, with gradients computed layer-by-layer backward via the chain rule — each layer's gradient depends on the gradient of the layer after it. This is what makes training deep networks computationally tractable.</p>
          </div>

          <p style="margin-top: 4px; color: var(--muted);">⚠️ <strong>Common pitfall:</strong> A "neural network with no hidden layer and no nonlinearity" is just linear/logistic regression in disguise — the hidden layers plus nonlinear activations are precisely what give MLPs their extra representational power.</p>
        `
      },
      {
        id: 'ml-09',
        ...
        content: `
          <div class="def-box">
            ...
            <p>High-dimensional data suffers from the <strong>curse of dimensionality</strong> — distances become less meaningful, and models need exponentially more data to generalize well. Dimensionality reduction compresses features while preserving as much useful structure as possible.</p>
          </div>

          <div class="theorem-box">
            <h5>Principal Component Analysis (PCA)</h5>
            <p>PCA finds new orthogonal axes (principal components) that capture maximum variance in the data, in decreasing order of importance.</p>
            <p><strong>Algorithm:</strong></p>
            <ol>
              <li>Center the data (subtract the mean of each feature).</li>
              <li>Compute the covariance matrix $\\Sigma = \\frac{1}{n}X^TX$.</li>
              <li>Find eigenvalues/eigenvectors of $\\Sigma$.</li>
              <li>The eigenvector with the largest eigenvalue is PC1 (direction of maximum variance); the next-largest orthogonal eigenvector is PC2, and so on.</li>
            </ol>
          </div>

          <div class="example-box">
            <h4>Interpreting Eigenvalues</h4>
            <p>Each eigenvalue $\\lambda_i$ equals the variance captured along that principal component. The fraction of total variance explained by the top k components is $$\\frac{\\sum_{i=1}^k \\lambda_i}{\\sum_{i=1}^d \\lambda_i}$$ — commonly plotted as a "scree plot" to decide how many components to keep.</p>
          </div>

          <div class="def-box">
            <h4>PCA via SVD</h4>
            <p>In practice, PCA is usually computed via the Singular Value Decomposition of the (centered) data matrix $X = U\\Sigma V^T$ rather than explicitly forming the covariance matrix — numerically more stable, and the right singular vectors (columns of V) are exactly the principal components.</p>
          </div>

          <p style="margin-top: 4px; color: var(--muted);">⚠️ <strong>Common pitfall:</strong> PCA is unsupervised and only looks at variance in the features — it has no knowledge of class labels, so the direction of maximum variance is not guaranteed to be the best direction for classification (that's what LDA is for).</p>
        `
      }
    ],
    quiz: [
      ...
      {
        q: 'Naive Bayes assumes that, given the class label, the features are:',
        opts: ['Perfectly correlated', 'Conditionally independent', 'Normally distributed only', 'Linearly separable'],
        ans: 1,
        exp: 'The "naive" assumption is conditional independence of all features given the class, which lets the joint likelihood factor into a simple product.'
      },
      {
        q: 'What makes LDA\'s decision boundary linear (unlike QDA)?',
        opts: ['It uses a linear activation function', 'It assumes all classes share the same covariance matrix', 'It ignores the class means', 'It only works on 2D data'],
        ans: 1,
        exp: 'LDA assumes a common covariance matrix across classes; this shared-covariance assumption is exactly what cancels the quadratic terms and leaves a linear decision boundary.'
      },
      {
        q: 'In Leave-One-Out Cross-Validation (LOO-CV) on a dataset with n samples, how many models are trained?',
        opts: ['1', 'n', 'n-1', 'n²'],
        ans: 1,
        exp: 'LOO-CV is k-fold CV with k = n: each of the n data points is held out once, so exactly n models are trained.'
      },
      {
        q: 'Why can\'t a Multi-Layer Perceptron with only linear activations represent XOR?',
        opts: ['XOR needs more than 2 inputs', 'Stacked linear layers collapse into a single linear transformation', 'MLPs cannot use gradient descent', 'XOR is not a classification problem'],
        ans: 1,
        exp: 'Without nonlinear activations between layers, any composition of linear layers is still just one linear function — which cannot represent the non-linearly-separable XOR function.'
      },
      {
        q: 'In PCA, the first principal component is the direction that:',
        opts: ['Minimizes variance in the data', 'Maximizes variance in the data', 'Is orthogonal to all class labels', 'Always aligns with the first original feature'],
        ans: 1,
        exp: 'PC1 is defined as the eigenvector of the covariance matrix with the largest eigenvalue — the direction along which the data has maximum variance.'
      }
    ]
  },
  ...
  AI: {
    ...
    lessons: [
      ...
      {
        id: 'ai-06',
        title: 'Reasoning Under Uncertainty: Exact & Approximate Inference',
        content: `
          <p>Given a Bayesian Network, the core computational task is <strong>inference</strong>: computing $P(\\text{Query} | \\text{Evidence})$. This lesson goes one level deeper than ai-03 into how that's actually computed.</p>

          <div class="theorem-box">
            <h5>Variable Elimination — Step by Step</h5>
            <p>To compute $P(X|e)$, we sum out ("eliminate") every variable that isn't the query or evidence, one at a time:</p>
            <ol>
              <li>Express the joint as a product of conditional probability factors (one per node).</li>
              <li>Restrict factors to observed evidence values.</li>
              <li>Pick a hidden variable, multiply together all factors mentioning it, then sum it out — producing one new combined factor.</li>
              <li>Repeat until only the query variable's factor(s) remain; normalize to get a valid probability distribution.</li>
            </ol>
            <p>Reusing intermediate factors (rather than expanding the full joint) is what makes this far cheaper than brute-force enumeration — though worst-case complexity is still exponential in the network's "tree width."</p>
          </div>

          <div class="example-box">
            <h4>Elimination Order Matters</h4>
            <p>The size of intermediate factors — and hence the total cost — depends heavily on the order variables are eliminated in. A good heuristic (e.g., eliminate the variable creating the smallest new factor) can make the difference between tractable and intractable inference on the same network.</p>
          </div>

          <div class="def-box">
            <h4>Approximate Inference via Sampling</h4>
            <p><strong>Rejection sampling:</strong> generate full samples from the network, discard any that disagree with the evidence, and estimate $P(Query|evidence)$ from what remains. Simple but wasteful if evidence is rare.</p>
            <p><strong>Likelihood weighting:</strong> fixes evidence variables to their observed values instead of sampling them, weighting each sample by how likely that evidence was given the sampled parents — no samples are wasted, but weights can still have high variance.</p>
            <p><strong>Gibbs sampling (MCMC):</strong> repeatedly resamples one non-evidence variable at a time, conditioned on the current values of all others (its Markov blanket), producing a chain of samples whose distribution converges to the true posterior.</p>
          </div>

          <p style="margin-top: 4px; color: var(--muted);">⚠️ <strong>Common pitfall:</strong> Exact inference (variable elimination) always gives the correct answer but can be exponentially expensive on densely connected networks; sampling methods trade guaranteed correctness for a scalable, "good enough with enough samples" approximation — pick based on network size and accuracy needs, not just habit.</p>
        `
      }
    ],
    quiz: [
      ...
      {
        q: 'In variable elimination, what makes it more efficient than brute-force enumeration of the full joint distribution?',
        opts: ['It ignores some variables entirely', 'It reuses intermediate factors instead of expanding the full joint table', 'It only works on trees, never general graphs', 'It approximates rather than computing exactly'],
        ans: 1,
        exp: 'Variable elimination multiplies and sums out relevant factors one variable at a time, reusing intermediate results — this avoids ever materializing the full exponential joint distribution, while still computing an exact answer.'
      },
      {
        q: 'In likelihood weighting, why are evidence variables fixed rather than sampled?',
        opts: ['To speed up random number generation', 'So that no generated samples are wasted/rejected for disagreeing with evidence', 'Because evidence variables have no parents', 'To guarantee convergence in one sample'],
        ans: 1,
        exp: 'By clamping evidence variables to their observed values and weighting samples by how likely that evidence was, likelihood weighting avoids the wasted computation of rejection sampling, where samples inconsistent with evidence are simply thrown away.'
      }
    ]
  }
};
